...
export default async function GameDayPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const gameDay = await prisma.gameDay.findUnique({
    ...
    include: {
      registrations: {
        ...
        include: {
          user: { select: { id: true, name: true, profile: { select: { photoUrl: true, stars: true } } } },
        },
      },
      teams: {
        orderBy: { name: "asc" },
        include: {
          players: {
            include: {
              user: {
                select: { id: true, name: true, profile: { select: { photoUrl: true, stars: true } } },
              },
            },
          },
        },
  ...
  const liveHref = admin ? `/peladas/${id}/painel` : `/peladas/${id}/ao-vivo`;
